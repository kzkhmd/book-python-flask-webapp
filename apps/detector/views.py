...
def detect(image_id):
    ...
    try:
        ...
        flash("物体検知中にエラーが発生しました。")
        db.session.rollback()
        current_app.logger.error(f"データベースエラー: {e}")
        return redirect(url_for("detector.index"))
    return redirect(url_for("detector.index"))
...
def draw_lines(c1, c2, result_image, color, line):
...
def exec_detect(target_image_path):
    ...
    for box, label, score in zip(output["boxes"], output["labels"], output["scores"]):
        if score > 0.5 and labels[label] not in tags:
            ...
            cv2 = draw_lines(c1, c2, result_image, color, line)
            cv2 = draw_texts(result_image, line, c1, cv2, color, labels, label)
    
    detected_image_file_name = str(uuid.uuid4()) + ".jpg"
    detected_image_path = str(Path(current_app.config["UPLOAD_FOLDER"], detected_image_file_name))
